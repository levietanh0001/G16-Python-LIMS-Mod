...
def viewUsers(frame_view_users):
        # connect to database
    ...
    if num_of_rows > 20:
        num_of_rows = 20
        # show frame_view_users
    frame_view_users.tkraise()
    frame_view_users.pack(fill=BOTH, expand="yes")

    trv = ttk.Treeview(frame_view_users, columns=(1,2,3,4,5), height=num_of_rows)
    style = ttk.Style(trv)
    style.configure('Treeview', background="black",
                        fieldbackground="black", foreground="white", bordercolor="black", borderwidth=0,
                        rowheight=27)
    ...
    for i in rows:
        ...
        # quit button
    quit_button = Button(frame_view_users,text="Back",bg='black', fg='white', command=frame_view_users.pack_forget)
