...
def viewBorrowersByBookName(frame_view_borrowers_by_book_name):
    global bookID, bookTitle, bookAuthor, bookStatus, cv, con, cur, book_table, root, bookName

        # show frame_view_books
    frame_view_borrowers_by_book_name.tkraise()
    frame_view_borrowers_by_book_name.pack(fill=BOTH, expand="yes")

    lb2 = Label(frame_view_borrowers_by_book_name, text="Book Title ", bg='black', fg='white')
    lb2.place(relx=0.05, rely=0.4)

    bookName = Entry(frame_view_borrowers_by_book_name)
    bookName.place(relx=0.2, rely=0.4, relwidth=0.7, relheight=0.1)

        # quit button
    submit_button = Button(frame_view_borrowers_by_book_name,text="Submit",bg='black', fg='white', command=view_borrowers_by_book_name)
    submit_button.place(relx=0.3,rely=0.84, relwidth=0.2, relheight=0.08)

        # quit button
    quit_button = Button(frame_view_borrowers_by_book_name,text="Back",bg='black', fg='white', command=frame_view_borrowers_by_book_name.pack_forget)
